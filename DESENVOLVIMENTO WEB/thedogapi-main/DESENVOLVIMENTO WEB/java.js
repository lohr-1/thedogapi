...
async function carregarGaleria() {
  try {
    ...
    // Promises para carregar todas as imagens antes de criar os cards
    const promises = racas.map(async (raca) => {
      ...
      if (raca.image?.url) {
        ...
        try {
          ...
          console.error(`Erro ao buscar imagem para a raça ${raca.name}:`, error);
        }
      }
    ...
    // Cria os cards após carregar as imagens
    ...
    // Após carregar os cards, aplique o efeito Parallax
...
async function carregarListaRacas() {
  try {
    const response = await fetch("https://api.thedogapi.com/v1/breeds");
    if (!response.ok) throw new Error("Erro ao carregar lista de raças");
    const racas = await response.json();
